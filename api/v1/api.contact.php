...
$app->group('/contact', function () use ($app) {
	...
	$app->post('/send-mail', function() use ($app) {
		...
		$response = send_template_mail('contact.html', $destination, 'Allgemeine Anfrage', $hooks, $sender_name);
	...
	$app->post('/send-mail-question', function() use ($app) {
		...
		$hooks = ['searchStrs' => ['#MESSAGE#', '#MAIL#', '#USERNAME#', '#AUTHOR#', '#QUESTION#', '#QUESTION_ID#', '#SUBJECT#', '#DATE2#', '#EXAM_ID#', '#MAIL_SUBJECT#'],
			'subjectStrs' => [$text, $email, $name, $data->author, $data->question, $data->question_id, $data->subject, $data->date, $data->exam_id, $mail_subject_html]];
		$response = send_template_mail('contact-question.html', $destination, $subject, $hooks, $sender_name);
